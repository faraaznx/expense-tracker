...
async def parse_receipt(
    ...
    # Upload to temp storage first (so images are safe even if parsing fails)
    temp_paths: list[str] = []
    for img_bytes in image_bytes_list:
        path = f"receipts/temp/{uuid.uuid4()}.jpg"
...
CONFIRM_PAYLOAD = {
    "temp_image_paths": ["receipts/temp/abc123.jpg"],
    "store_name": "Lulu Hypermarket",
    "date": "2026-04-28",
    "total_aed": "25.00",
    "source": "physical_photo",
...
async def confirm_receipt(
    ...
    async with db.begin():
        ...
        for i, temp_path in enumerate(request.temp_image_paths):
            perm_path = f"receipts/{user_id}/{receipt.id}/{i}.jpg"
...
async def delete_receipt(
    ...
    """Delete a receipt and all its children (cascade).
    ...
    result = await db.execute(
        select(Receipt).where(
            Receipt.id == receipt_id, Receipt.user_id == uuid.UUID(user_id)
        )
    )
    receipt = result.scalar_one_or_none()
    if not receipt:
